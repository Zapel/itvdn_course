...
class Api(_Api):
    ...
    def _get_privat_rate(self, from_currency):
        response = self._send_request("https://api.privatbank.ua/p24api/pubinfo?exchange&json&coursid=11", method="get")
    ...
    def _find_rate(self, response_data, from_currency):
        ...
        if from_currency not in privat_aliases_map:
            raise AttributeError(f"Invalid from_currency: {from_currency}")
